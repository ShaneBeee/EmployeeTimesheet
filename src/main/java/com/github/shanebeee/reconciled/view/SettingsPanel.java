...
public class SettingsPanel extends JPanel {
    ...
    private JPanel buildEmployeeInfoTab(EmployeeInfo info) {
        ...
        card.add(makeFieldRow("Full Name", nameField));
        card.add(makeDivider());
        card.add(makeFieldRow("Company", companyField));
        card.add(makeDivider());
        card.add(makeFieldRow("Address", addressField));
        card.add(makeDivider());
        card.add(makeFieldRow("Address 2", address2Field));
        card.add(makeDivider());
        card.add(makeFieldRow("Phone", phoneField));
        card.add(makeDivider());
        ...
        calcRow.add(homeCalc, BorderLayout.WEST);
    ...
    private JPanel buildPreferencesTab() {
        ...
        card.add(makeDivider());
        card.add(makeFieldRow("Default Start Time", startField));
        card.add(makeDivider());
    ...
    private JPanel buildProfilesTab() {
        ...
        JPanel card = makeCard();
        card.setLayout(new BoxLayout(card, BoxLayout.Y_AXIS));
        card.add(makeSectionRow("User Profiles"));
        card.add(makeDivider());
        ...
        if (profileManager == null) {
            ...
            Runnable refresh = () -> {
                ...
                for (UserProfile p : profiles) {
                    Color ac;
                    try {
                        ac = Color.decode(p.getAvatarColor());
                    } catch (Exception ex) {
                        ac = new Color(59, 130, 246);
                    }
    ...
    private void showProfileEditDialog(UserProfile p, Runnable[] refreshRef) {
        ...
        try {
            ac[0] = Color.decode(p.getAvatarColor());
        ...
        hdrLeft.add(avatarPreview);
        hdrLeft.add(hdrTitle);
        ...
        JPanel colorSwatch = new JPanel() {
            ...
            protected void paintComponent(Graphics g) {
                ...
                g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
                try {
                    g2.setColor(Color.decode(colorField.getText().trim()));
                } catch (Exception ex) {
        ...
        colorRow.add(colorSwatch, BorderLayout.EAST);
        ...
        addFormRow(form, gc, "Avatar Colour", colorRow);
        gc.gridy++;
        addFormRow(form, gc, "Display Name", pNameField);
        gc.gridy++;
        addFormRow(form, gc, "Company", pCompanyField);
        gc.gridy++;
        addFormRow(form, gc, "Address", pAddressField);
        gc.gridy++;
        addFormRow(form, gc, "Address 2", pAddress2Field);
        gc.gridy++;
        addFormRow(form, gc, "Phone", pPhoneField);
        gc.gridy++;
        addFormRow(form, gc, "Email", pEmailField);
        gc.gridy++;
        addFormRow(form, gc, "Office Area (sq ft)", pOfficeSqFt);
        gc.gridy++;
        ...
        saveBtn.addActionListener(e -> {
            ...
            try {
                pInfo.setHomeOfficeSqFt(Double.parseDouble(pOfficeSqFt.getText().trim()));
            } catch (NumberFormatException ignored) {
            }
            try {
                pInfo.setHomeTotalSqFt(Double.parseDouble(pHomeSqFt.getText().trim()));
            } catch (NumberFormatException ignored) {
            }
    ...
    private void showCategoryDialog(ExpenseCategory existing) {
        ...
        gbc.gridy++;
        gbc.insets = new Insets(0, 0, 4, 0);
        form.add(makeDialogLabel("Hint"), gbc);
        gbc.gridy++;
        gbc.insets = new Insets(0, 0, 12, 0);
        form.add(hintField, gbc);
        gbc.gridy++;
        gbc.insets = new Insets(0, 0, 4, 0);
        form.add(makeDialogLabel("T2125 Line"), gbc);
        gbc.gridy++;
        gbc.insets = new Insets(0, 0, 12, 0);
        form.add(t2125Field, gbc);
        gbc.gridy++;
        gbc.insets = new Insets(0, 0, 4, 0);
        form.add(makeDialogLabel("Colour"), gbc);
        gbc.gridy++;
        gbc.insets = new Insets(0, 0, 12, 0);
        ...
        deductionCombo.addActionListener(e -> {
            boolean show = deductionCombo.getSelectedIndex() == 1;
            percentLabel.setVisible(show);
            percentField.setVisible(show);
            dialog.revalidate();
            dialog.repaint();
        });
        ...
        btnSave.addActionListener(e -> {
            ...
            if (name.isBlank()) {
                JOptionPane.showMessageDialog(dialog, "Name is required.", "Required", JOptionPane.WARNING_MESSAGE);
                return;
            }
            cat.setLabel(name);
            ...
            if (dtype == ExpenseCategory.DeductionType.FIXED_PERCENT) {
                try {
                    ...
                    JOptionPane.showMessageDialog(dialog, "Enter a valid % (1-100).", "Invalid", JOptionPane.WARNING_MESSAGE);
                    return;
                }
            }
    ...
    private JPanel buildTaxBracketsTab() {
        ...
        JSpinner.NumberEditor ye = new JSpinner.NumberEditor(yearSpinner, "#");
        ye.getTextField().setHorizontalAlignment(JTextField.CENTER);
        yearSpinner.setEditor(ye);
        yearRow.add(yearLbl);
        ...
        cnt[0].setLayout(new BoxLayout(cnt[0], BoxLayout.Y_AXIS));
        cnt[0].setOpaque(false);
        cnt[0].setAlignmentX(LEFT_ALIGNMENT);
        Runnable build = () -> {
            cnt[0].removeAll();
            ...
            cppCard.add(makeFieldRow("Combined Rate (%)", cppRate));
            cppCard.add(makeDivider());
            cppCard.add(makeFieldRow("Annual Maximum ($)", cppMax));
            cppCard.add(makeDivider());
            JPanel hr = new JPanel(new BorderLayout());
            hr.setOpaque(false);
            hr.setBorder(BorderFactory.createEmptyBorder(4, 0, 4, 0));
            hr.add(cppHint, BorderLayout.WEST);
            cppCard.add(hr);
            ...
            saveCpp.addActionListener(e -> {
                try {
                    brackets.setCppRate(Double.parseDouble(cppRate.getText().trim()) / 100.0);
                    brackets.setCppMaxContribution(Double.parseDouble(cppMax.getText().trim()));
                    storage.saveTaxBrackets(brackets);
                    JOptionPane.showMessageDialog(this, "CPP saved.", "Saved", JOptionPane.INFORMATION_MESSAGE);
                } catch (NumberFormatException ex) {
                    JOptionPane.showMessageDialog(this, "Invalid numbers.", "Error", JOptionPane.WARNING_MESSAGE);
                }
            });
            JPanel cppBtn = new JPanel(new FlowLayout(FlowLayout.RIGHT, 0, 6));
            cppBtn.setOpaque(false);
            cppBtn.add(saveCpp);
            cppCard.add(cppBtn);
            cnt[0].add(cppCard);
            cnt[0].add(Box.createVerticalStrut(12));
            cnt[0].add(makeBracketCard("Federal Income Tax Brackets", brackets.getFederal(), brackets, true));
            cnt[0].add(Box.createVerticalStrut(12));
        ...
        yearSpinner.addChangeListener(e -> {
            bh[0] = storage.loadTaxBrackets((int) yearSpinner.getValue());
            build.run();
        });
        tab.add(cnt[0]);
        return wrapInScroll(tab);
    ...
    private JPanel makeBracketCard(String title, java.util.List<com.github.shanebeee.reconciled.model.TaxBrackets.Bracket> brackets, com.github.shanebeee.reconciled.model.TaxBrackets tb, boolean isFederal) {
        ...
        Runnable refreshRows = () -> {
            ...
            for (int i = 0; i < brackets.size(); i++) {
                final int idx = i;
                com.github.shanebeee.reconciled.model.TaxBrackets.Bracket b = brackets.get(i);
                boolean isLast = i == brackets.size() - 1;
                JPanel row = new JPanel(new java.awt.GridLayout(1, 3, 8, 0));
                row.setOpaque(false);
                row.setBorder(BorderFactory.createEmptyBorder(4, 0, 4, 0));
                row.setMaximumSize(new Dimension(Integer.MAX_VALUE, 32));
                JTextField upTo = new JTextField(isLast ? "No limit" : String.format("%.0f", b.getUpTo()));
                upTo.setEnabled(!isLast);
                ...
                del.setEnabled(brackets.size() > 1);
                ...
                row.add(del);
                rowsPanel.add(row);
                upTo.addFocusListener(new java.awt.event.FocusAdapter() {
                    public void focusLost(java.awt.event.FocusEvent e) {
                        if (!isLast) try {
                            b.setUpTo(Double.parseDouble(upTo.getText().trim()));
                        } catch (NumberFormatException ignored) {
                        }
                    }
                });
                rate.addFocusListener(new java.awt.event.FocusAdapter() {
                    public void focusLost(java.awt.event.FocusEvent e) {
                        try {
                            b.setRate(Double.parseDouble(rate.getText().trim()) / 100.0);
                        } catch (NumberFormatException ignored) {
                        }
                    }
                });
        ...
        btnAdd.addActionListener(e -> {
            double prev = brackets.size() > 1 ? brackets.get(brackets.size() - 2).getUpTo() + 10000 : 50000;
            brackets.add(brackets.size() - 1, new com.github.shanebeee.reconciled.model.TaxBrackets.Bracket(prev, 0.20));
            refreshRows.run();
        });
        ...
        btnSave.addActionListener(e -> {
            ...
            if (isFederal) tb.setFederal(brackets);
            else tb.setBc(brackets);
            storage.saveTaxBrackets(tb);
